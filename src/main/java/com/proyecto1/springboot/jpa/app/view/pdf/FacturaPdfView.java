package com.proyecto1.springboot.jpa.app.view.pdf;
...
import com.proyecto1.springboot.jpa.app.models.entity.Factura;
import com.proyecto1.springboot.jpa.app.models.entity.ItemFactura;
...
public class FacturaPdfView extends AbstractPdfView{
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		tabla2.addCell(mensajes.getMessage("text.cliente.factura.fecha") + ": " + factura.getCreadoEn());
		...
		for(ItemFactura item: factura.getItems()) {
			...
			tabla3.addCell(df.format(item.getProducto().getPrecio()).toString());
	
			//Usar el objeto cell para poder centrar el texto de la columna cantidad
			cell = new PdfPCell(new Phrase(item.getCantidad().toString()));
			cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
			tabla3.addCell(cell);
			
			tabla3.addCell(df.format(item.calcularImporte()).toString());
		}
		...
		tabla3.addCell(cell);
		tabla3.addCell(df.format(factura.getTotal()).toString());
